...
## 2026-09-23 – `b5a3a98` Transaktionen: ESPNs echtes Transaktions-Log statt Roster-Diffing

Nutzer-Frage: weshalb fehlt Woche 2 bei den Transaktionen komplett?

Root Cause: Transaktionen wurden bisher durch Roster-Diffing rekonstruiert
(aktueller Kader-Snapshot gegen den vom letzten Lauf verglichen), jede neu
gefundene Differenz mit der AKTUELLEN ESPN-Matchup-Periode ZUM ZEITPUNKT
DES SYNC-LAUFS gestempelt - nicht mit der Woche, in der sie tatsächlich
passiert ist. Der automatische Sync läuft nur wöchentlich (Dienstags):
letzter Lauf während Woche 2 aktiv war am 16.9., danach kein Lauf bis
22.9. - zu dem Zeitpunkt zeigte ESPN schon Woche 3 an. Alle Änderungen
vom 16.-21.9. (echte Woche 2) wurden dadurch fälschlich als "Woche 3"
gespeichert, "Woche 2" fehlte komplett. Nichts ging verloren, nur falsch
beschriftet.

Fix: ESPNs eigenes Transaktions-Log (view=mTransactions2) statt Roster-
Diffing. Wichtige API-Eigenart dabei entdeckt (per Debug-Workflow verifiziert,
danach entfernt): scoringPeriodId filtert exakt auf GENAU DIESE Periode,
keine kumulative Historie - fetchAllTransactions() fragt deshalb jede Woche
1..currentWeek einzeln ab und fügt zusammen. buildTransactionsFromLog()
übersetzt ESPNs Rohformat:
- FREEAGENT (status EXECUTED): sofortige Adds/Drops, ADD+DROP im selben
  Eintrag werden zu einer Zeile mit Zusatztext kombiniert (wie bisher).
- WAIVER (executionType PROCESS): EXECUTED -> Waiver Claim erfolgreich,
  FAILED_* -> WAIVER_FAILED. PENDING/CANCELED (vom Manager zurückgezogen)
  werden bewusst nicht angezeigt, da kein Endergebnis.
- Trades laufen über mehrere verknüpfte Einträge (TRADE_PROPOSAL trägt die
  Spieler-Items, TRADE_UPHOLD macht ihn nach der Review-Frist rechtskräftig
  - kommt 1x pro beteiligtem Team, hier über relatedTransactionId dedupliziert
  -, TRADE_DECLINE bei Ablehnung). relatedTransactionId zeigt auf die id des
  ursprünglichen Proposals, das über alle Wochen hinweg nachgeschlagen wird.
- ROSTER (reine Lineup-Änderungen) und DRAFT (Draft-Picks) werden ignoriert.

Behält die alte id-Präfix-Konvention ('add-'/'drop-' für FREEAGENT) bei,
da findWaiverKarmaFact()/findWaiverInstantSuccessFact() weiter oben im
Skript genau danach filtern.

roster-snapshot.json und das gesamte Diff-Apparat (gainedByTeam/lostByTeam/
isFirstRun) sind damit überflüssig und komplett entfernt - transactions.json
wird jetzt bei jedem Lauf deterministisch aus ESPNs Log neu gebaut statt
inkrementell fortgeschrieben (kein Merge mit alten Einträgen mehr nötig).

Offline mit Mock-Daten getestet, die exakt den echten ESPN-Antworten
nachgebildet sind (per Debug-Workflow gegen die echte Liga gesammelt):
FREEAGENT solo/mit Drop-Partner, WAIVER erfolgreich/gescheitert/pending/
zurückgezogen, Trade-Korrelation über 2 UPHOLD-Einträge auf 1 Anzeige-
Eintrag dedupliziert, ROSTER/DRAFT-Rauschen korrekt ignoriert - alle
Fälle wie erwartet.
