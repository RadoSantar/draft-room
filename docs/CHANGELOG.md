...
## 2026-09-15 – `4b4718a` Recaps: 13 neue Fakten-Kategorien - zielt direkt auf die Woche-1-Fakten-Knappheit

Nach der Diskussion über redundante Recaps: die Analyse hatte gezeigt,
dass Woche 1 nur eine Handvoll "immer verfügbarer" Fakten-Kategorien hat
(Standout, Tabellenplatz, Bank-Reue), weil vieles andere Saison-Historie
braucht, die es in Woche 1 noch nicht gibt. Diese 13 neuen Kategorien
erweitern den Pool gezielt in zwei Richtungen:

Gruppe A - brauchen NICHTS, funktionieren schon ab Woche 1:
- powerRankMovement: Power-Ranking-Sprung ggü. Vorwoche (bzw. "post-draft"
  in Woche 1) - Daten lagen in power-rankings-history.json schon brach.
- leagueBestPosition: bester Spieler einer Position in der GANZEN Liga
  diese Woche (nicht nur im eigenen Spiel wie der bestehende "standout").
- marginExtreme: knappstes/deutlichstes Spiel der Woche (Marge statt Score).
- positionalFlop: Gegenstück zu positionalDominance - eine Positionsgruppe
  liefert fast nichts.
- waiverInstantSuccess: ein DIESE Woche geholter Spieler liefert sofort.
- chalk: Favorit gewinnt fast exakt mit der prognostizierten Marge.

Gruppe B - brauchen Saison-Historie (ab Woche 2-3 sinnvoll):
- formTrend: 3 Wochen in Folge steigende/fallende Punktzahl.
- positionalSlump: Positionsgruppe dauerhaft unter Liga-Schnitt.
- seasonBenchTotal: kumulierte, auf der Bank liegengelassene Saison-Punkte.
- tradeImpact: ein per Trade geholter Spieler liefert fürs neue Team.
- consistency: geringste Wochen-zu-Wochen-Schwankung der Liga.

Gruppe C - eigene Ideen:
- upsetTally: Anzahl Aussenseiter-Siege dieses Teams über die Saison.
- perfectWeekProximity: Starter ganz nah an der eigenen Saison-Bestleistung.

Technisch:
- scripts/sync-espn.mjs: 13 neue find*Fact()-Funktionen + neue
  archiveSeasonStats() (analog zu archiveLiveSnapshotWeek, eigener
  weeksArchived-Guard, läuft aber unabhängig von Live-Snapshots) schreibt
  data/season-stats.json fort: pro Team kumulierte Bank-Lücke,
  Aussenseiter-Siege, Positions-Wochentotale; pro Spieler die bisherige
  Saison-Bestleistung. TRADE-Transaktionen bekommen jetzt strukturierte
  "legs" (wer kriegt welche Spieler-IDs), FREEAGENT-Adds bekommen
  teamId/playerId - beides vorher nur Fliesstext, jetzt zusätzlich
  maschinenlesbar für tradeImpact/waiverInstantSuccess.
- scripts/generate-recaps.mjs: alle 13 neuen Kategorien in collectFacts()
  verdrahtet (inkl. Kategorie-Tag für die bestehende Wochen-Deckelung -
  läuft also automatisch durch dieselbe Redundanz-Vermeidung wie die
  bisherigen Kategorien) + passende Spitzname-Pools in BADGE_POOL.
- Mit gemockter API getestet: alle 13 neuen Fakten werden korrekt in den
  Prompt aufgenommen, respektieren die Wochen-Obergrenze, Wochen-Recap
  funktioniert unverändert.

data/season-stats.json neu angelegt (startet leer, wird ab jetzt jede
Woche fortgeschrieben - Gruppe-B-Fakten werden dadurch von Woche zu Woche
zuverlässiger). data/game-recaps.json und data/week-recaps.json für
Woche 1 nochmal geleert, damit der erweiterte Fakten-Pool direkt zum
Tragen kommt.
