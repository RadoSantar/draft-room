...
## 2026-09-15 – `c14d3b5` Recaps: Fakten-Kategorien werden jetzt über die Woche hinweg gedeckelt

Feedback nach dem echten Woche-1-Lauf: die Recaps fühlten sich redundant
an, weil dieselbe Art Fakt (typischerweise Bank-Reue) in fast jedem Spiel
der Woche auftauchte - bestätigt sich direkt im gecachten Woche-1-Output:
4 von 5 Einzel-Recaps UND der Wochenüberblick erwähnen alle prominent
liegengelassene Bank-Punkte, obwohl >20 andere Fakten-Kategorien
existieren (Standout, Playoff-Rennen, Erwartungswert, Draft-Reue,
Team-Storyline, etc.).

Root Cause: collectFacts() lieferte pro Spiel eine seedbasiert gemischte
Zufallsauswahl - aber jedes Spiel wurde unabhängig von den anderen
gewählt, ohne zu wissen, was die übrigen Spiele der Woche schon zeigen.
Bank-Reue tritt in echten Matchups einfach sehr häufig auf (fast jedes
Team lässt IRGENDWO Punkte liegen), landet deshalb oft in der
Zufallsauswahl und wird so zum De-facto-Standardfakt jeder Woche.

Fix:
- collectFacts() taggt jeden Fakt jetzt mit einer Kategorie (z.B.
  "loserBenchRegret", "standout", "expectation").
- Neue pickFactsForGame(game, categoryUsage, ...) wählt Fakten pro Spiel,
  bevorzugt aber Kategorien, die diese Woche noch nicht 2x verwendet
  wurden (categoryUsage wird über alle Spiele des Laufs mitgeführt).
  Reicht die unverbrauchte Auswahl für ein Spiel nicht aus, wird mit den
  am wenigsten überstrapazierten Kategorien aufgefüllt statt den Recap
  faktenlos zu lassen.
- generateRecapsForGames() und buildWeekPrompt() nutzen das jetzt statt
  der alten unkoordinierten Pro-Spiel-Zufallsauswahl (mit getrennten
  categoryUsage-Zählern, da beide Prompts unabhängig funktionieren).
- Mit gemockter Claude-API getestet: bei künstlich fakten-armen Spielen
  (nur 6 Kategorien für 5 Spiele à 3 Fakten) wird die Kategorie-Obergrenze
  zwar am Ende der Woche unvermeidbar überschritten, aber deutlich
  seltener als vorher (Bank-Reue 5/5 -> 3/5) - mit dem echten, deutlich
  breiteren Fakten-Pool pro Spiel sollte die Obergrenze in der Praxis
  fast immer eingehalten werden.

data/game-recaps.json und data/week-recaps.json für Woche 1 geleert,
damit sie mit der neuen Logik sauber neu generiert werden.
